...
from scenes.gameplay.testscene import TestScene
from scenes.mainmenu.settings import Settings
from scenes.mainmenu.credits import Credits
        ...
        for event in events:
            if event.type == pygame.MOUSEBUTTONDOWN: # If the user clicks
                ...
                if self.play_text_rect.collidepoint(mouse_pos):
                    self.game.game_state_stack.change_state(TestScene)
                    return True
                elif self.settings_text_rect.collidepoint(mouse_pos):
                    self.game.game_state_stack.push(Settings)
                    return True
                elif self.credits_text_rect.collidepoint(mouse_pos):
                    self.game.game_state_stack.push(Credits)
                    return True
